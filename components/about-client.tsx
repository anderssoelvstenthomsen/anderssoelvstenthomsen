...
export default function AboutClient({ settings }: { settings: SiteSettings }) {
  ...
  const paragraphs = settings.aboutBio.split(/\n{2,}/).map((p) => p.trim()).filter(Boolean);
  const [intro, ...more] = paragraphs;

  const groups = [
    { label: "Clients", items: settings.clientsList },
    { label: "Art Direction", items: settings.artDirectionList },
    { label: "Editorial", items: settings.editorialList },
  ];
